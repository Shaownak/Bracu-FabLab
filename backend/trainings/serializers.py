...
class TrainingCourseListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'title', 'slug', 'description', 'category', 'thumbnail', 'duration_hours', 'is_required', 'total_lessons', 'user_progress']
    def get_user_progress(self, obj):
        request = self.context.get('request')
        if request and request.user.is_authenticated:
            try:
                progress = UserTrainingProgress.objects.get(user=request.user, course=obj)
                return {'progress': progress.progress_percentage, 'quiz_passed': progress.quiz_passed, 'completed': progress.completed_at is not None}
...
class TrainingCourseDetailSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = TrainingCourse
        fields = ['id', 'title', 'slug', 'description', 'category', 'thumbnail', 'duration_hours', 'is_required', 'total_lessons', 'lessons', 'quiz']
...
class UserTrainingProgressSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'course', 'course_title', 'progress_percentage', 'quiz_score', 'quiz_passed', 'started_at', 'completed_at']
